set_game_instance();
if (room == rm_start) {
	if (audio_is_playing(msc_start)) audio_stop_sound(msc_start);
 	if (allow_playing_sound) audio_play_sound(msc_start, 1, false);
	...
	if (audio_is_playing(msc_game)) audio_stop_sound(msc_game);
	if (allow_playing_sound) audio_play_sound(msc_game, 2, true);
	alarm[0] = 60;
	global.playing = true;
}