...
function encounter_player(observable_distance_to_player, closed_distance_to_player, speed_bullet, original_speed){
	...
	if (distance_between_player <= observable_distance_to_player) { // 350
		...
		if (distance_between_player <= closed_distance_to_player) { // 200
			speed -= 0.05
		} else   {
			speed += 0.01
		}
		...
		if (bullet_counter >= bullet_spawing_latency) {
			spawn_bullet(id, image_angle, speed_bullet, faction, color); // TODO: make the bullet speed as a variable
			bullet_counter = 0;
		}
	} else {
